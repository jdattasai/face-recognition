...
im_eye=im(e_s_r:e_e_r,e_s_c:e_e_c,1:3);
im_no=im(bbox(18):bbox(18)+bbox(20),bbox(17):bbox(17)+bbox(19),1:3);
im_mo=im(bbox(14):bbox(14)+bbox(16),bbox(13):bbox(13)+bbox(15),1:3);
[re,ce,pe]=size(im_eye);
[rn,cn,pn]=size(im_no);
[rm,cm,pm]=size(im_mo);
tran_im=zeros(re+rn+rm,max([ce,cn,cm]));

tran_im(1:re,1:ce,1:3)=im_eye;
tran_im(re+1:re+rn,1:cn,1:3)=im_no;
tran_im(re+rn+1:re+rn+rm,1:cm,1:3)=im_mo;
